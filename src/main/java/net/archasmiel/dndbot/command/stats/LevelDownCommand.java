...
public class LevelDownCommand extends Command {
  ...
  public static final Command INSTANCE = new LevelDownCommand();
  private static final SlashCommandData DATA =
      Commands.slash("35leveldown", "Персонаж отримує -1 рівень та оновлює макс. ману");

  private LevelDownCommand() {
  ...
  public void process(SlashCommandInteraction interaction) {
    ...
    try {
      ...
      user.setLevel(user.getLevel() <= 1 ? 1 : user.getLevel()-1);
